...
use crate::state::{
    LastUsedMove, Pokemon, PokemonBoostableStat, PokemonMoveIndex, PokemonSideCondition,
...
impl Pokemon {
    pub fn gen3_add_available_moves(
        &self,
    ...
    pub fn gen3_add_move_from_choice(&self, vec: &mut Vec<MoveChoice>, choice: Choices) {
        let mut iter = self.moves.into_iter();
        while let Some(p) = iter.next() {
            if p.id == choice {
                vec.push(MoveChoice::Move(iter.pokemon_move_index));
    ...
    pub fn gen3_has_type(&self, pkmn_type: &PokemonType) -> bool {
        pkmn_type == &self.types.0 || pkmn_type == &self.types.1
    }

    pub fn gen3_item_can_be_removed(&self) -> bool {
    ...
    pub fn gen3_is_grounded(&self) -> bool {
        if self.gen3_has_type(&PokemonType::FLYING) || self.ability == Abilities::LEVITATE {
            return false;
        }
        true
    }

    pub fn gen3_volatile_status_can_be_applied(
        &self,
        ...
        match volatile_status {
            PokemonVolatileStatus::LEECHSEED => {
                if self.gen3_has_type(&PokemonType::GRASS)
    ...
    pub fn gen3_immune_to_stats_lowered_by_opponent(
        &self,
...
impl Side {
    pub fn gen3_active_is_charging_move(&self) -> Option<PokemonMoveIndex> {
    ...
    pub fn gen3_get_boost_from_boost_enum(&self, boost_enum: &PokemonBoostableStat) -> i8 {
        match boost_enum {
    ...
    pub fn gen3_calculate_boosted_stat(&self, stat: PokemonBoostableStat) -> i16 {
    ...
    pub fn gen3_has_alive_non_rested_sleeping_pkmn(&self) -> bool {
    ...
    pub fn gen3_add_switches(&self, vec: &mut Vec<MoveChoice>) {
    ...
    pub fn gen3_trapped(&self, opponent_active: &Pokemon) -> bool {
            ...
            .contains(&PokemonVolatileStatus::PARTIALLYTRAPPED)
        {
            ...
            return true;
        } else if opponent_active.ability == Abilities::ARENATRAP && active_pkmn.gen3_is_grounded()
        {
            return true;
...
impl State {
    pub fn gen3_root_get_all_options(&self) -> (Vec<MoveChoice>, Vec<MoveChoice>) {
        ...
        let (mut s1_options, mut s2_options) = self.gen3_get_all_options();
    ...
    pub fn gen3_get_all_options(&self) -> (Vec<MoveChoice>, Vec<MoveChoice>) {
        ...
        if self.side_two.force_switch {
            self.side_two.gen3_add_switches(&mut side_two_options);
        ...
        {
            side_one_options.push(MoveChoice::None);
        } else if let Some(mv_index) = self.side_one.gen3_active_is_charging_move() {
            ...
            if !self.side_one.gen3_trapped(side_two_active) {
                self.side_one.gen3_add_switches(&mut side_one_options);
            }
        }
        ...
        {
            side_two_options.push(MoveChoice::None);
        } else if let Some(mv_index) = self.side_two.gen3_active_is_charging_move() {
            ...
            if !self.side_two.gen3_trapped(side_one_active) {
                self.side_two.gen3_add_switches(&mut side_two_options);
            }
        }
    ...
    pub fn gen3_remove_volatile_statuses_on_switch(
        &mut self,
        side_ref: &SideReference,
    ...
    pub fn gen3_weather_is_active(&self, weather: &Weather) -> bool {
    ...
    fn gen3_state_contains_any_move(&self, moves: &[Choices]) -> bool {
    ...
    pub fn gen3_set_damage_dealt_flag(&mut self) {
        if self.gen3_state_contains_any_move(&[
            Choices::COUNTER,
            Choices::MIRRORCOAT,
            Choices::FOCUSPUNCH,
        ]) {
    ...
    pub fn gen3_set_last_used_move_flag(&mut self) {
        if self.gen3_state_contains_any_move(&[Choices::ENCORE, Choices::FAKEOUT]) {
            self.use_last_used_move = true
        }
    }

    pub fn gen3_set_conditional_mechanics(&mut self) {
        /*
        ...
        */
        self.gen3_set_damage_dealt_flag();
        self.gen3_set_last_used_move_flag();
    }
}
