pub fn generate_instructions_with_state_assertion(
    ...
    let instructions =
        generate_instructions_from_move_pair::<GEN>(state, side_one_move, side_two_move, false);
...
fn set_moves_on_pkmn_and_call_generate_instructions(
    ...
    state
        .side_one
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M0, move_one);
    state
        .side_two
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M0, move_two);
...
fn test_branch_on_crit() {
    ...
    let vec_of_instructions = generate_instructions_from_move_pair::<GEN>(
        &mut state,
        &MoveChoice::Move(PokemonMoveIndex::M0),
        &MoveChoice::Move(PokemonMoveIndex::M0),
        true,
...
fn test_branch_when_a_roll_can_kill() {
    if !(GEN == 9 || cfg!(feature = "champions")) { return; }
    let mut state = State::default();
...
fn test_knockoff_boosts_damage_but_cannot_remove_if_stickyhold() {
    if !(cfg!(feature = "champions") || GEN == 9 || GEN == 8 || GEN == 7 || GEN == 6) { return; }
    let mut state = State::default();
    state.side_one.get_active().item = Items::LEFTOVERS;
...
fn test_earlier_gen_nothing_happens_if_destinybond_is_used_while_already_having_destinybond() {
    if !(GEN == 3 || GEN == 4 || GEN == 5 || GEN == 6) { return; }
    let mut state = State::default();
    state.side_one.get_active().speed = 150;
    state
        .side_one
...
fn test_locked_moves_unlock_on_switchout() {
    ...
    state
        .side_two
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M0, Choices::SPLASH);
...
fn test_switchout_flag_where_faster_switchout_move_knocked_out_opponent() {
    ...
    state.side_two.get_active().moves[&PokemonMoveIndex::M0] = Move {
        ...
        disabled: false,
        pp: 35,
        choice: moves::<GEN>().get(&Choices::TACKLE).unwrap().clone(),
    };
...
fn test_pkmn_is_not_trapped_if_it_has_fainted() {
    ...
    let (side_one_moves, side_two_moves) = state.get_all_options::<GEN>();
...
fn test_noretreat_traps_pokemon() {
    ...
    let (side_one_moves, _side_two_moves) = state.get_all_options::<GEN>();
...
fn test_meanlook_traps_opponent_from_switching() {
    ...
    let (_side_one_moves, side_two_moves) = state.get_all_options::<GEN>();
...
fn test_meteorbeam_volatile_only_allows_meteor_beam_to_be_used() {
    ...
    state.side_one.get_active().moves[&PokemonMoveIndex::M0] = Move {
        ...
        disabled: false,
        pp: 16,
        choice: moves::<GEN>().get(&Choices::METEORBEAM).unwrap().clone(),
    };
    let (side_one_moves, _side_two_moves) = state.get_all_options::<GEN>();
...
fn test_assaultvest_prevents_status_move() {
    ...
    state.side_one.get_active().moves[&PokemonMoveIndex::M0] = Move {
        ...
        disabled: false,
        pp: 35,
        choice: moves::<GEN>().get(&Choices::TOXIC).unwrap().clone(),
    };
    ...
    let (side_one_moves, _) = state.get_all_options::<GEN>();
    assert_eq!(
        vec![
            MoveChoice::Move(PokemonMoveIndex::M1),
            MoveChoice::Move(PokemonMoveIndex::M2),
...
fn test_pressure_caused_double_pp_decrement() {
    ...
    state
        .side_one
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M0, Choices::TACKLE);
...
fn test_pressure_does_not_cause_pp_decrement_if_move_targets_self() {
    ...
    state
        .side_one
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M0, Choices::RECOVER);
...
fn test_pp_not_decremented_when_flinched() {
    ...
    state
        .side_two
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M0, Choices::BITE);
    state.side_two.get_active().moves[&PokemonMoveIndex::M0].pp = 1;
...
fn test_healing_wish_is_consumed_even_if_it_does_nothing() {
    if !(GEN == 4 || GEN == 5 || GEN == 6 || GEN == 7) { return; }
    let mut state = State::default();
    state.side_one.side_conditions.healing_wish = 1;
...
fn test_guaranteed_to_stay_asleep_sleeptalk_move_when_not_rested() {
    ...
    state
        .side_one
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M0, Choices::SLEEPTALK);
    state
        .side_one
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M1, Choices::REST);
    state
        .side_one
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M2, Choices::TACKLE);
    state
        .side_one
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M3, Choices::CURSE);
...
fn test_fast_taunt_gets_applied_and_duration_increments() {
    if !(GEN == 5 || GEN == 6 || GEN == 7 || GEN == 8 || GEN == 9) { return; }
    let mut state = State::default();
...
fn test_futuresight_activating() {
    if !(cfg!(feature = "champions") || GEN == 9 || GEN == 8 || GEN == 7 || GEN == 6) {
        return;
    }
    let mut state = State::default();
...
fn test_toxic_count_is_reset_even_if_toxic_is_reapplied_the_same_turn() {
    let mut state = State::default();
    let mut toxic_choice = moves::<GEN>().get(&Choices::TOXIC).unwrap().to_owned();
    ...
    state
        .side_two
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M0, Choices::REFRESH);
...
fn test_mimikyu_with_disguise_formechange_on_damaging_move() {
    ...
    if GEN == 8 || GEN == 9 || cfg!(feature = "champions") {
...
fn test_pursuit_into_switch() {
    ...
    state.side_one.get_active().moves.m0 = Move {
        ...
        disabled: false,
        pp: 35,
        choice: moves::<GEN>().get(&Choices::PURSUIT).unwrap().to_owned(),
    };
...
fn test_lifeorb_hitting_sub() {
    ...
    let expected_instructions = if GEN == 4 {
        vec![StateInstructions {
        percentage: 100.0,
        instruction_list: vec![
            Instruction::DamageSubstitute(DamageInstruction {
...
fn test_prankster_into_dark_type() {
    if !(cfg!(feature = "champions") || GEN == 9 || GEN == 8 || GEN == 7) { return; }
    let mut state = State::default();
    state.side_one.get_active().ability = Abilities::PRANKSTER;
...
#[test]
fn test_drizzle() {
    if !(GEN == 6 || GEN == 7 || GEN == 8 || GEN == 9) { return; }
    let mut state = State::default();
...
#[test]
fn test_drizzle_earlier_gens() {
    if !(GEN == 3 || GEN == 4 || GEN == 5) { return; }
    let mut state = State::default();
    state.side_one.pokemon[PokemonIndex::P1].ability = Abilities::DRIZZLE;
...
#[test]
fn test_unseenfist() {
    if cfg!(feature = "champions") { return; }
    let mut state = State::default();
...
fn test_transistor_higher_boost_before_gen8() {
    if !(GEN == 8 || GEN == 7 || GEN == 6 || GEN == 5 || GEN == 4) { return; }
    let mut state = State::default();
    state.side_one.get_active().ability = Abilities::TRANSISTOR;
...
fn test_sleeptalk_when_asleep_and_rest_turns_active() {
    ...
    state
        .side_one
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M0, Choices::REST);
    state
        .side_one
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M1, Choices::SLEEPTALK);
    state
        .side_one
        .get_active()
        .replace_move::<GEN>(PokemonMoveIndex::M2, Choices::TACKLE);
...
fn test_mustrecharge_move_only_allows_none() {
    ...
    let options = state.get_all_options::<GEN>();
